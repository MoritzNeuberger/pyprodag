...
def check_required_entries_in_settings(input):
    ...
    for req in requirements_base:
        if(not req in input):
            raise Exception("Required parameter {} not set. Exit.".format(req))
    ...
    for creq_k, creq_v in conditional_requirements.items():
        if(creq_k in input["work_flow"] and input["work_flow"][creq_k] and not creq_v in input ):
            raise Exception("Conditionally required parameter {} not set. It is required to set when {} is given. Exit.".format(creq_v,creq_k))
    ...
    for creq_k, creq_v in conditional_work_flow_requirements.items():
        if(creq_k in input["work_flow"] and input["work_flow"][creq_k] and not creq_v in input["work_flow"] ):
            raise Exception("Conditionally required parameter {} not set. It is required to set when {} is given. Exit.".format(creq_v,creq_k))
