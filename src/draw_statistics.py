...
def draw_ratio_status_running_over_time(summary, folder_name):
    if(len(summary)>0):
        axis = summary.plot(x="time",y="ratio_status_running_over_time",color= cset.black)
        axis.grid(visible=True,axis="both",which="both")
...
def draw_total_duration_of_jobs(finished_info,folder_name):
    if(len(finished_info)>0):
        axis = finished_info.plot.hist(column=["time_since_creation"],bins=10 ** np.linspace(np.log10(finished_info["time_since_creation"].min()), np.log10(finished_info["time_since_creation"].max()), 50),color=cset.red)
        axis.grid(visible=True,axis="both",which="both")
