...
def generate_folder_and_file_name(settings):
    ...
    if (
        ...
        folder_name["detailed"] = (
            settings["work_flow"]["cwd"]
            + settings["work_flow"]["folder_detailed_output"]
            + time.strftime(r"" + hard_coded_settings["date_format"], loc_time)
            + "/"
        )
        file_name["detailed"] = (
            "root_information_"
            + time.strftime(r"" + hard_coded_settings["date_and_time_format"], loc_time)
            + ".json"
        )
    ...
    if (
        ...
        folder_name["finished"] = (
            settings["work_flow"]["cwd"] + settings["work_flow"]["folder_finished_jobs"]
        )
        file_name["finished"] = (
            "finished_root_information_"
            + time.strftime(r"" + hard_coded_settings["date_format"], loc_time)
            + ".json"
        )
    ...
    if (
        ...
        folder_name["summary"] = (
            settings["work_flow"]["cwd"] + settings["work_flow"]["folder_summary"]
        )
        file_name["summary"] = (
            "summary_"
            + time.strftime(r"" + hard_coded_settings["date_format"], loc_time)
    ...
    if (
        "with_graphical_output" in settings["work_flow"]
        and settings["work_flow"]["with_graphical_output"]
    ):
        folder_name["graphs"] = (
            settings["work_flow"]["cwd"]
            + settings["work_flow"]["folder_graphs"]
            + time.strftime(r"" + hard_coded_settings["date_format"], loc_time)
            + "/"
        )
...
def extract_timestamp_from_file_name(file_name):
    ...
    return int(
        time.mktime(
            datetime.strptime(
                old_date_and_time, hard_coded_settings["date_and_time_format"]
            ).timetuple()
        )
    )
...
def search_for_old_folders(folder_name):
    ...
    for subdir, dirs, files in os.walk(base_folder):
        for dir in dirs:
            if os.path.normpath(subdir + "/" + dir) != os.path.normpath(
                folder_name["detailed"]
            ):
