...
def get_info_of_individual_finished_processes(finished_jobs_pid,previous_data,folder_name,file_name,settings):
    if(len(previous_data) > 0 and len(finished_jobs_pid) > 0):
        output = list()
        for pid in finished_jobs_pid:
            output.append(generate_dict_with_info_finished(pid,previous_data,settings))
        finished_data = pd.DataFrame(output)

        if(path.isfile(folder_name+file_name)): 
            with open(folder_name+file_name,'r') as json_file:
...
def write_out_finished_job_information(finished_data_individual: pd.DataFrame, finished_data_summary: pd.DataFrame,folder_name,file_name):
    print("len(finished_data_individual):", len(finished_data_individual), " - len(finished_data_summary):", len(finished_data_summary))
    if(len(finished_data_individual)>0 and len(finished_data_summary)>0):
        json_file = {"individual_finished_data": finished_data_individual.to_dict(), "summary_finished_data": finished_data_summary.to_dict()}
